...
const ValidationOutput = (props) => {
    const { usage, user, setaboutq, aboutq, setSumarizationText,setChatData, chatData, lastAnswer, isDark}:any = UserAuth();
...
// const about = user.about == undefined ? [] : user.about
  ...
  return  (
    <div className={`  border ${isDark ? 'border-[#000] rounded-box bg-[#000] text-white':'border-[#fff] text-black rounded-box bg-[#fff]'} p-8 shadow-three dark:bg-gray-dark sm:p-11 lg:p-8 xl:p-11   `}>
...
<h3 className="my-5 text-2xl font-bold leading-tight">
        Answer Length
      </h3>
        <ul className="items-center justify-center flex flex-wrap">
          {user.about.map((item) => item.length <= 10 ? (
            <li className={`${active === item ? 'bg-gradient-to-r from-blue-600 via-green-500 to-indigo-400 inline-block text-transparent  rounded-box px-5 py-2 m-2 ':'bg-[#333] text-[#f0f0f0] rounded-box px-5 py-2 m-2'} cursor-pointer`} 
            onClick={() => {
            setActive(item)
            setaboutq(item)
          }}
          ...
          <li className={`${active === '' ? 'bg-gradient-to-r from-primary/60 via-green-500 to-primary/60 inline-block text-transparent  rounded-box px-5 py-2 m-2 ':'bg-[#333] text-[#f0f0f0] rounded-box px-5 py-2 m-2'} cursor-pointer`}
           onClick={() => {
            setActive('')
            setaboutq('')
          }}>Default</li>
...
<h3 className="py-5 text-2xl font-bold leading-tight dark:text-white">
       Summarize Answer
      </h3>
