...
import {  FaImage,  FaUserAlt, FaUserAstronaut,   } from 'react-icons/fa';
import Markdown from 'react-markdown'

function ChatComponent(props) {
    const chatEndRef = useRef(null)
    const { usage,Token,tokenLoading, setusage,usagErrorMessage, authToken, user,aboutq, sumarizationText, chatData, setChatData,question, message, setMessage,lastAnswer, setLastAnswer, isDark}:any = UserAuth();
...
const speek = () => {      
 //   say.speak('Hi', 'Alex', 1.0);
  ...
  const handleFileChange = (event) => {
    ...
    return (
        <>
            <div className={`max-w-3xl items-center justify-end flex ${isDark ? 'bg-black':'bg-fff'}  `} style={{ flexDirection: 'column',}}>
...

                {
                    tokenLoading ? (
                        ...
                        <div className=' items-center justify-center flex ' >
                            ...
                            <div
                                // onSubmit={() => { }}
                                className={`${isDark ? 'rounded-xl bg-black   px-2':'rounded-xl bg-white   px-2'}`}
                                style={{}}
                            >
                                ...
                                {chatData.map((item) => (
                                    <div>
                                        {item.ans && (
                                            <div className="chat chat-start mb-3 mt-3">
                                                ...
                                                <div className="chat-header pb-3 flex items-center justify-between" style={{flexDirection:'row'}}>
                                                    Geesi AI 
                                                    <p className={`mx-3 px-1.5 py-0.5 ${isDark ? 'bg-[#fff] text-black':'bg-primary text-white '} cursor-pointer text-black rounded-box`} onClick={() => {
                                                      setCopied(true)
                                                ...
                                                <div className={`chat-bubble   ${isDark ? 'bg-[#333] text-white':'bg-[#fff] text-black border-2'}`}
                                                ...
                                                <div className="chat-image avatar">
                                                     {user.photoURL == '' ? (
                                                      <div className='w-10 mt-5'>
                                                           <FaUserAstronaut size={25} className='text-white/60' />
                                                        </div>
                                                     ):(
                                                    <div className="w-10 rounded-full">
                                                           <Image
                ...
                flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed "
                           ...
                           <span className="loading loading-ring loading- text-white size-12"></span>
                                        ...
                                        }} className={`${isDark ? 'bg-black text-white px-5 hover:bg-black border-black hover:border-black':'bg-white text-primary px-5 hover:bg-white border-white hover:border-white border-0'} hover:text-primary/60   py-3 font-bold`}>
                                    Answer
                                </button>
                                )}
