...
const NewsLatterBox = () => {
  ...
  const {user, authToken}:any = UserAuth()
  // subscribe
  ...
  const [laoding, setLoading] = useState(false)
      ...
      axios.post(`${EndPoint}/api/subscribe`, {email, authToken})
                        .then((response) => {
  ...
  return (
    <div className="relative z-10 border border-[#000] rounded-box bg-black p-8 shadow-three dark:bg-gray-dark sm:p-11 lg:p-8 xl:p-11">
      <h3 className="mb-4 text-2xl font-bold leading-tight text-white dark:text-white">
      ...
      <div>
        <input
          ...
          disabled={laoding}
          onClick={subscribe}
          value={laoding ? 'Loading...':responsemessage}
          className="mb-5 rounded-box flex w-full cursor-pointer items-center justify-center border border-white bg-black px-9 py-4 text-base font-medium text-white shadow-submit duration-300 hover:bg-black/90 dark:shadow-submit-dark"
        />
